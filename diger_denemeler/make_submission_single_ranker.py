...
def _print(msg: str):
    print(f"[submit_single_ranker] {msg}")
...
def build_features(df: pd.DataFrame):
    ...
    # Drop datetime columns and typical time-named fields to avoid time leakage
    time_name_keywords = (
        "ts_",
        "_ts",
        "date",
...
def main(root: Path, out_name: str = "submission_single_ranker.csv"):
    models_dir = root / "models" / "catboost_ranker"
    meta_p = models_dir / "meta_weighted.json"
    if not meta_p.exists():
        raise FileNotFoundError(f"{meta_p} bulunamadı. Önce train_single_ranker_weighted.py ile model eğitin.")
    ...
    model_path = Path(meta.get("model_path", models_dir / "model_ranker_weighted.cbm"))
    model = CatBoostRanker(); model.load_model(str(model_path))

    scores = model.predict(test_df[features])
...
def parse_args():
    ...
    p.add_argument("--out", type=str, default="submission_single_ranker.csv", help="Çıktı dosya adı")
    return p.parse_args()
